...
export default function Scoot() {
  ...
  // Live suggestions from track B's TomTom search while typing.
  useEffect(() => {
    const q = filter.trim();
    if (q.length < 3) { setSuggestions([]); return; }
    const timer = setTimeout(() => {
      searchSuggestions(q)
        .then((list) => setSuggestions(list as Destination[]))
  ...
  /** Free-text search — track B's resolveLocation (lat,lng or
      TomTom geocode), then the normal plan flow. */
  async function searchFree(query: string) {
    setStatus({ text: "Zoeken…", error: false });
    try {
      const point = await resolveLocation(query);
      if (!point) { setStatus({ text: "Geen locatie gevonden. Probeer een adres of lat, lng.", error: true }); return; }
      setStatus(null);
      plan({ name: query, area: "Gezocht", point });
    } catch {
      setStatus({ text: "Zoeken mislukt. Probeer het nog eens.", error: true });
    }
  }
